...
async function performDateTransitions() {
    // Handled fully on the Node backend server to prevent client discrepancies.
}
...
function handleNavigation() {
    ...
    navItems.forEach(item => {
        item.addEventListener("click", function(e) {
            ...
            // Remove active status from all elements (sidebar and mobile bar)
            navItems.forEach(nav => nav.classList.remove("active"));
            
            // Add active status to both desktop and mobile matches for this tab
            document.querySelectorAll(`[data-tab="${targetTab}"]`).forEach(el => {
...
// --- SUGGESTED QUESTS BOARD LOGIC ---
const SUGGESTED_QUESTS = [
    { title: "Morning Hydration: Drink 1 glass of water after waking up", category: "water", xp: 15, notes: "Kickstart your metabolism. Drink a full glass of water first thing in the morning." },
    { title: "Afternoon Hydration: Drink 1 glass of water before lunch", category: "water", xp: 15, notes: "Prevent midday fatigue by keeping your hydration buffer full." },
    { title: "Post-Workout Hydration: Drink 500ml water after exercise", category: "water", xp: 15, notes: "Rehydrate your muscles and recover body salts lost during training." },
    { title: "Sugar Ban: Zero refined sugar intake today", category: "food", xp: 25, notes: "Swap out sugary sodas and snacks for whole fruits or water." },
    { title: "Prep Green Fuel: Servings of green vegetables with meal", category: "food", xp: 20, notes: "Include healthy greens like broccoli or spinach to power your cells." },
    { title: "Protein Booster: Consume protein with breakfast", category: "food", xp: 20, notes: "Fuel your muscles and keep sated. Include eggs, Greek yogurt, or protein shakes." },
    { title: "Morning Cycling: 30 minutes outdoor bicycle ride", category: "workout", xp: 35, notes: "Ride your bicycle in the morning to oxygenate your brain and build leg strength." },
    { title: "Gym Training: Daily weight lifting session", category: "workout", xp: 35, notes: "Complete a structured strength training session at the gym." },
    { title: "Read 10 Pages: Daily book reading", category: "reading", xp: 20, notes: "Read at least 10 pages of non-fiction or development books to expand your knowledge." },
    { title: "Mindfulness Meditation: 15-minute sitting breathing session", category: "meditation", xp: 20, notes: "Sit in silent reflection, breathing deeply to calm mental noise." }
];

function renderSuggestedQuests() {
    const container = document.getElementById("suggested-quests-container");
    if (!container) return;

    container.innerHTML = SUGGESTED_QUESTS.map((quest, idx) => {
        const isAdded = state.tasks.some(t => t.title === quest.title);

        return `
            <div class="quest-card glow-blue ${isAdded ? 'completed' : ''}" style="border-style: ${isAdded ? 'solid' : 'dashed'};">
                <div class="quest-card-header">
                    <div>
                        <h3 class="quest-card-title" style="font-size: 0.95rem;">${quest.title}</h3>
                        <span class="quest-badge tag-${quest.category}">${quest.category}</span>
                    </div>
                </div>
                <div class="quest-card-body" style="font-size: 0.75rem;"><p>${quest.notes}</p></div>
                <div class="quest-card-meta">
                    <div class="quest-check-reward">+${quest.xp} XP base</div>
                </div>
                <div class="quest-card-actions" style="margin-top: auto; border-top: 1px solid rgba(255,255,255,0.05); padding-top: 0.75rem;">
                    ${isAdded ? 
                        `<button class="btn btn-secondary" style="width:100%; cursor:default; font-size:0.75rem;" disabled><i data-lucide="check-circle-2"></i> Accepted</button>` : 
                        `<button class="btn btn-accent btn-accept-suggested" data-idx="${idx}" style="width:100%; font-size:0.75rem;"><i data-lucide="plus-circle"></i> Accept Quest</button>`
                    }
                </div>
            </div>
        `;
    ...
    container.querySelectorAll(".btn-accept-suggested").forEach(btn => {
        btn.addEventListener("click", async function() {
            const idx = parseInt(this.getAttribute("data-idx"));
            await acceptSuggestedQuest(idx);
        });
    });
}

async function acceptSuggestedQuest(idx) {
    const quest = SUGGESTED_QUESTS[idx];
    const taskId = "t_sug_" + Date.now() + "_" + idx;
    const newQuest = { 
        id: taskId, 
        title: quest.title, 
    ...
    try {
        ...
        notifyNewLog(`Recommended Quest accepted: ${quest.title}`);
        SoundFX.playQuestComplete();
        
        renderQuests();
    } catch (err) {
        SoundFX.playError();
        alert("Failed to accept suggested quest.");
    }
}
...
async function toggleQuestComplete(taskId, completeStatus) {
    ...
    try {
        const response = await apiRequest(`/tasks/${taskId}`, {
            method: "PUT",
            body: JSON.stringify({ completedToday: completeStatus })
        });

        if (!response) return;

        if (completeStatus) {
            notifyNewLog(`Completed quest: ${task.title}. Gained +${response.xpAwarded} XP! (Streak bonus included)`);
            SoundFX.playQuestComplete();
            triggerConfettiEffect();
        } else {
            notifyNewLog(`Undo quest: ${task.title}. Deducted ${response.xpAwarded} XP.`);
            SoundFX.playError();
        }

        // Re-bootstrap entire state to sync refreshed user profile, XP/Level, achievements progress and histories
        await appBootstrap();

    } catch (err) {
        SoundFX.playError();
        alert("Failed to update quest status on backend.");
    }
}
...
function renderAnalytics() {
    ...
    logTbody.innerHTML = sortedHistory.map(item => {
        const isNegative = item.xp < 0;
        const xpText = isNegative ? `-${Math.abs(item.xp)} XP` : `+${item.xp} XP`;
        const xpColor = isNegative ? '#ff4d4f' : 'var(--gold)';
        return `
            <tr>
                <td>${new Date(item.timestamp).toLocaleString()}</td>
                <td style="${isNegative ? 'color:#8e99b3; font-style:italic;' : ''}">${item.title}</td>
                <td><span class="quest-badge tag-${item.category}">${item.category}</span></td>
                <td style="color:${xpColor}; font-weight:700;">${xpText}</td>
            </tr>
        `;
    }).join('');
}
